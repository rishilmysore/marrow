...
9. [x] **Post-push cold verify + evidence commit.** From a directory outside this repo: `git ls-remote --tags origin` lists v0.1.0; fresh `git clone https://github.com/rishilmysore/marrow.git`; `git checkout v0.1.0`; follow the README install cold into a second scratch repo; fast gate green inside it; confirm the friction-report form renders on GitHub's new-issue page. Then replace the two `deferred:` cells in the archived plan with the real evidence, commit `public-cut: post-push evidence`, push it (Boundary 1 covers this push).
   - Deviation (accepted residual): "form renders on GitHub's new-issue page" is not mechanically verifiable — GitHub exposes no API for YAML issue forms (GraphQL `issueTemplates` lists only .md templates; calibrated against cli/cli, whose forms don't appear there either) and /issues/new is login-gated for anonymous clients. Proxies all green (Verify row 8); the one-click manual check remains: open https://github.com/rishilmysore/marrow/issues/new?template=friction-report.yml while logged in.
...
| Check                             | How                                                                                                                                      | Evidence |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| 1 Fast gate + harness             | `sh adapters/lint.sh && ! grep -rn "README" templates/` exits 0; `sh adapters/lint_test.sh` ends `lint-test: 26 ok, 0 failing`           | At closeout: `marrow-lint: ok`, grep silent (exit 0), `lint-test: 26 ok, 0 failing`. Also run before every task commit. ✓ |
...
| 4 Shipped files untouched         | `git diff <plan-commit>..HEAD --stat -- templates/ adapters/` prints nothing                                                             | `git diff f9b88af..HEAD --stat -- templates/ adapters/` printed nothing at closeout (f9b88af = plan commit). ✓ |
| 5 Newcomer battery                | Per question: subagent's answer + README citation + verdict; defects fixed and rerun clean, or residuals recorded with reasons           | 12/12 correct on first pass, zero fix cycles, zero residuals — table at §Evidence — Task 6 below. Cold `sonnet` subagent, README-only guidance, fresh fixture install. ✓ |
| 6 Fixture install sound           | Merged AGENTS.md; violated rule handled per the README's documented path; markers `marrow v0 @ v0.1.0`; `marrow-lint: ok` in the fixture | Single merged AGENTS.md (3 house rules folded into Commands/Verification, Conventions, Boundaries); violated `set -euo pipefail` rule kept at AGENTS.md:51 + filed as STATE.md Next action; all 5 markers `marrow v0 @ v0.1.0`; `marrow-lint: ok` ran green inside the fixture; install committed (068f349). ✓ |
| 7 Tag + release published         | `git ls-remote --tags origin` shows v0.1.0; push output; release URL (or the recorded manual step)                                       | `ls-remote --tags` → `refs/tags/v0.1.0` (annotated 92df789 → commit 97ab660, the closeout commit); push output `4bf77c9..97ab660 main -> main` + `* [new tag] v0.1.0 -> v0.1.0` after clean dry-run; release live at https://github.com/rishilmysore/marrow/releases/tag/v0.1.0 (HTTP 200, notes = CHANGELOG [0.1.0] section, `--verify-tag`). ✓ |
| 8 Cold clone installs             | Fresh clone at v0.1.0, README-only install transcript summary, fast gate green, issue form renders                                       | Fresh `git clone` from GitHub outside this repo, `checkout v0.1.0` = 97ab660; README install into a fresh scratch repo: cp templates, fill `<Project>`+TODOs, seed Focus/Next action, pin 5 markers `marrow v0 @ v0.1.0`, commit — `marrow-lint: ok` inside it via the clone's adapters/lint.sh. Task 5's 6 deferred claims resolve: tag exists, release + compare/v0.1.0...HEAD + form-file URLs all HTTP 200. Form render: proxies green (YAML field-by-field schema check, file at v0.1.0 via contents API, hasIssuesEnabled, `friction-report` label created); logged-in browser render recorded as the accepted residual under Task 9. ✓ |
...
## Evidence — Task 6 battery (Verify row 5; cold `sonnet` subagent, README-only, after live install into the fixture)

| Q | Crit. | Answer (condensed) | Citation given | Verdict |
|---|---|---|---|---|
| 1 | a | Five files + one loop; minimal agent-agnostic framework; NOT a tool you install — markdown you copy; for any agent + the adopting human | README 3–5 | ✓ |
| 2 | b | Stash/commit existing AGENTS.md first → cp overwrites → merge rules into template's sections, don't keep both → violated rule stays, filed as STATE.md Next action → fill TODOs + `<Project>`, seed Focus/Next action, append tag to markers, commit install | Install 14–16 | ✓ |
| 3 | b | `marrow v0 @ v0.1.0` in each copied file's header comment | Install 16 | ✓ |
| 4 | c | CLAUDE.md containing `@AGENTS.md` (import guaranteed, "see X" advisory); read once at session start → edits land next session | Wiring 20 | ✓ |
| 5 | c | Nothing — "the files are the integration" | Wiring 21 | ✓ |
| 6 | c | Feed AGENTS.md into context each session; run `sh adapters/lint.sh` by hand or in CI | Wiring 22 | ✓ |
| 7 | d | risky-novel (new dependency = new external surface); create plans/rate-limiting.md from plans/_TEMPLATE.md, Spec before Tasks; listed in STATE.md In flight | Loop 34, 20 | ✓ |
| 8 | d | Planning fills a context with exploration the executor shouldn't inherit; committed plan file is the entire handoff | Loop 36 | ✓ |
| 9 | e | Teeth pre-commit via hook (worktree vs HEAD); CI: `sh adapters/lint.sh origin/main` makes it range-aware; one-liner lives in lint.sh's header (quoted it by following the README's pointer) | Lint 51 + lint.sh header | ✓ |
| 10 | f | `git -C marrow fetch --tags; git -C marrow diff v0.1.0 v0.2.0 -- templates/` (+ adapters/ if vendored); hand-apply; re-anchor markers to `marrow v0 @ v0.2.0` | Upgrading 65–72 | ✓ |
| 11 | g | Friction report form (Feedback link); Moment of silence ×2, Leak count = 2; 0.x — reports decide what changes and are the 1.0 gate | Feedback 76–80 | ✓ |
| 12 | g | Not stable: 0.x = shape set, contract not frozen; breaking may land in MINOR (flagged, never PATCH); 1.0 = 3 instrumented adoptions + 2 contract-stable MINORs + 1 real upgrade report | Versioning 61, 76 | ✓ |
